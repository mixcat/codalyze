...
public class BlinkMComm {
	...
	public String[] listPorts() {
		...
		if( osname.toLowerCase().startsWith("windows") ) {
			...
			for(int i=0;i<a.length/2;i++){
				String t = a[i];
				a[i] = a[a.length-(1+i)]; a[a.length-(1+i)] = t;
			}
	...
	// /dev/tty.usbserial-A60049Ch 
	// new BlinkMComm().connect(CompoundConsole.simulator, "/dev/tty.usbserial-A60049Ch");
...
*/
	...
	/*
	 bmc = new BlinkMComm();
bmc.connect(CompoundConsole.simulator, "/dev/tty.usbserial-A60049Ch");
bmc.stopPlayingScript();
	 */
