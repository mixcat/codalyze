...
public class Blinkm {
			...
			for (int i=0; i<this.customScriptLength; i++) {
				...
				if (ticks <= currentScriptTick && currentScriptTick < ticks+lineTicks  ) {
					...
					if (currentScriptTick == ticks) {
						Definition def = Definition.getByChar((char) line[1]);
	...
	protected static double getFadeComponent(double from, double to, byte fadeSpeed) {
		...
		if (diff == 0)
			return to;
		int steps = 255-fadeSpeed-1;
		double step = diff/steps;
		double result = (to>=from) ? Math.ceil(from + step) : Math.floor(from+step);
	...
	public void setCmd(byte[] cmd) {
		byte defChar = cmd[0];
		Definition definition = Definition.getByChar((char) defChar);
